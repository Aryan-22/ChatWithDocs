...
class ConversationalRAG:
    def __init__(self,session_id:str,retreiver) -> None:
        try:
            self.log = CustomLogger().get_logger(__name__)
        except Exception as e:
            self.log.error("Error initializing ConversationalRAG",error = str(e),session_id = session_id)
            raise DocumentPortalException("Failed to initialise ConversationRAG",sys)
    ...
    def _get_session_history(self):
        try:
            ...
            raise DocumentPortalException("Failed to retrieving session history",sys)
